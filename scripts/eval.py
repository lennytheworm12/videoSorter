...
Ratings feed scripts.calibrate which fits w1/w2 weights for source_score and cluster_score.

Usage:
    uv run python -m scripts.eval                          # rate all unrated eval_queries
    uv run python -m scripts.eval "cassiopeia into zed"    # rate a one-off question
    uv run python -m scripts.eval --add                    # add a question to eval_queries
    uv run python -m scripts.eval --stats                  # show rating distribution
"""
